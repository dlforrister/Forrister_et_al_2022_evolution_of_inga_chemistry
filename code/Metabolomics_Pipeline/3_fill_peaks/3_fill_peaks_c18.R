...
features <- read.csv("./data/C18/c18_compound_feature_table.csv",stringsAsFactors=FALSE)
...
pkl <- read.csv("./data/C18/c18_comp_feat_pcid_mzrt_rtwindow.csv",row.names = 1)
...
sb <- read.csv("./data/C18/C18_column_samples_quality_reviewed.csv")
...
filled_features_output_path <- paste("./data/C18/fill_feat/",remaining_samples$species[n],"_",gsub(".mzXML","",basename(remaining_samples$mzxml[n])),"_filled_features_c18.csv",sep="")

# make sure sample hasn't already been run
species_ran <- gsub("_filled_features_c18.csv","",list.files("./data/C18/fill_feat/"))
...
if (just_samp %in% sample_blank_RTI$file_name){
  ...
  rti_tic <- dbGetQuery(mydbold, paste("SELECT RTI, Standard, TIC_into from `RTI_DB` where RTI like '%c18%'", sep = ""))
...
filenames <- list.files(path="./data/C18/fill_feat/",pattern="*.csv")

## Full path to csv filenames
fullpath=file.path("./data/C18/fill_feat/",filenames)

## Merge listed files from the path above
dataset <- do.call("rbind",lapply(fullpath,FUN=function(files){ read.csv(files)}))

## write new CSV 
write.csv(dataset,"./data/C18/fill_feat/filled_features_c18_allsamples.csv",row.names=F)