...
compound_feature_table <- read.csv("./data/polar/polar_comp_feat_pcid_mzrt_rtwindow.csv")
...
species_ran <- gsub(".mgf","",gsub("all_msms_spectra_","",list.files("./data/polar/spectral_hits/matched_compounds/",pattern = ".mgf")))
...
species_ms_ms_table$column <- "polar"
...
write(text_to_write, file=paste("./data/polar/spectral_hits/matched_compounds/all_msms_spectra_", species[i], ".mgf", sep = ""))
...
write.csv(species_ms_ms_table_to_extract_msms[,c("species","scan_number","feature_number","compound_number","TIC","cluster_mz","cluster_rt","column","matched","matched","n_match_per_comp","matched_actual")],file=paste("./data/polar/spectral_hits/matched_compounds/csv/all_msms_spectra_", species[i], ".csv", sep = ""),row.names = F)
