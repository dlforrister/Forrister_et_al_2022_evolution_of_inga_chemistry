...
## read in matrix for all compounds by species name with 40% cutoff
all_sampsbycomps_species <- read.csv("./data/0_combined/final_sampcomps/5_sampsbycomps_combined_40cutoff_species_2020_10_7.csv",row.names=1)
names(all_sampsbycomps_species) <- gsub("X","",names(all_sampsbycomps_species))
row.names(all_sampsbycomps_species)
...
feature_info <- read.table(file = '/uufs/chpc.utah.edu/common/home/inga-group1/4_directories_chem_evolution_2019_03_26/data/00_C18/all_comps_info_V3_2020_5_4_knowns_anchors.csv', sep = ',', header = TRUE)
...
write.csv(similarity_matrix_sap,"./data/0_combined/matrices_for_MaJo/update/allcomps_speciesname_saponin_similarity_2020_10_07.csv",row.names=T)
...
write.csv(similarity_matrix_phen,"./data/0_combined/matrices_for_MaJo/update/allcomps_speciesname_phenolic_similarity_2020_10_07.csv",row.names=T)
...
spec_key <- dbGetQuery(mydb,"SELECT species_code,species_name FROM `inga_plantspecies` WHERE genus in ('Inga','Zygia')")

spec_key$species_code[spec_key$species_code=="M27"] <- "M27b"
spec_key$species_code[spec_key$species_code=="T67"] <- "T67b"
...
comp.class.pcts <- data.frame("sample" = row.names(sampsByCompounds), 
                              ...
                              "species_name" = sapply(1:nrow(sampsByCompoundsPhen), 
...
comp.class.pcts <- join(comp.class.pcts,spec_key,by="species_name",type="left",match="all")
comp.class.pcts$species_code[comp.class.pcts$sample=="Zygia mediana"] <- "T76"
comp.class.pcts$species_code[comp.class.pcts$sample=="M30"] <- "M8"
comp.class.pcts$species_code[comp.class.pcts$sample=="M27b"] <- "M27b"
comp.class.pcts <- comp.class.pcts[!comp.class.pcts$sample=="no phylogney",]
...
comp.class.pcts <- comp.class.pcts[!is.na(comp.class.pcts$percent_extracted),]

comp.class.pcts2 <- data.frame("sample"=as.character(unique(comp.class.pcts$sample)))
comp.class.pcts2 <- merge(comp.class.pcts2,comp.class.pcts[,1:8],by="sample",all.y=F)

library(dplyr)
comp.class.pcts2 <- distinct(comp.class.pcts2)
comp.class.pcts2$percent_extracted <- 0
comp.class.pcts2$percent_tyrosine <- 0

for (i in 1:nrow(comp.class.pcts2)){
  comp.class.pcts2$percent_extracted[i] <- mean(comp.class.pcts$percent_extracted[comp.class.pcts$sample==comp.class.pcts2$sample[i]])
  comp.class.pcts2$percent_tyrosine[i] <- mean(comp.class.pcts$percent_tyrosine[comp.class.pcts$sample==comp.class.pcts2$sample[i]])
}

comp.class.pcts2$percent_extracted[comp.class.pcts2$sample=="T21"] <- extr.pct$percent_extracted[extr.pct$species_code=="T21a"]
comp.class.pcts2$percent_tyrosine[comp.class.pcts2$sample=="T21"] <- tyr.pct$percent_tyrosine[tyr.pct$species_code=="T21a"]

comp.class.pcts <- comp.class.pcts2
...
for (i in 1:nrow(pairwise.spp)){
  for (j in 1:nrow(pairwise.spp)){
    pairwise.spp[i,j] <- pairwise.spp[i,j] / max(max(pairwise.spp[i,i],pairwise.spp[j,j]),pairwise.spp[i,j])
    pairwise.spp[j,i] <- pairwise.spp[i,j]
  }
}

write.csv(pairwise.spp, "./data/0_combined/matrices_for_MaJo/update/allcomps_speciesname_allchem_similarity_2020_10_09.csv", row.names = TRUE)

## without tyrosine
pairwise.spp <- similarity_matrix_phen*pairwise.phen.min + similarity_matrix_sap*pairwise.sap.min
...
write.csv(pairwise.spp, "./data/0_combined/matrices_for_MaJo/update/allcomps_speciesname_allchem_notyrosine_similarity_2020_10_09.csv", row.names = TRUE)
